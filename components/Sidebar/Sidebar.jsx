...
export default function Sidebar() { 
    ...
    return (
        <div ref={menuRef} onClick={() => sidebar.open()}
            className={`menu ${sidebar.isOpen || dropdown.isMobileOpen ? 'lg:w-[511px]' : 'lg:w-[90px]'} border-black border-0
            ...
            <div className='flex flex-col gap-6 lg:gap-10 w-full'>
                ...
                duration-300 ${sidebar.isOpen || dropdown.isMobileOpen ? 'delay-300' : 'delay-0'} relative max-h-16`}>
                    <LogoTSPK styles='fill-white hidden lg:block w-[50px] h-16' click={e => e.stopPropagation()}/>
                    <p style={{
                        transition: 'all 0.4s ease-out', 
                        transitionDelay: `${sidebar.isOpen || dropdown.isMobileOpen ? 1 * 50 + 200 : 0}ms`,
                    }}
                        className={`${sidebar.isOpen || dropdown.isMobileOpen ? 'opacity-100' : 'opacity-0'} 
                ...
                <div className={`bg-white/5 rounded-[10px] p-4 flex items-center gap-2.5 mt-10 lg:mt-0
                    transition-all duration-500 ${sidebar.isOpen || dropdown.isMobileOpen ? 'delay-100' : 'delay-0'}`}>
                    ...
                    <input 
                        ...
                        transition-all duration-300 ${sidebar.isOpen || dropdown.isMobileOpen ? 'delay-200' : 'delay-0'}`}
                    />
                </div>
                                ...
                                <li key={index} className='min-w-full'>
                                    <div className="border-0 border-transparent cursor-pointer bg-white/10 leading-[135%] flex w-full items-center lg:max-h-12.5 text-base rounded-[10px] p-3.75">
                                        <Link className={`flex items-center gap-2.5 transition-all text-white
                                            duration-300 ${sidebar.isOpen || dropdown.isMobileOpen ? 'delay-300' : 'delay-0'}`} href={item.link} onClick={e => e.stopPropagation()}>
                                            <Image src={item.imgUrl} alt={item.name} className='size-5 max-w-5 max-h-5 mr-0.5'/>
                                            <p style={{transition: 'all 0.4s ease-out', transitionDelay: `${sidebar.isOpen || dropdown.isMobileOpen ? 1 * 50 + 200 : 0}ms`}}
                                                className={`${sidebar.isOpen || dropdown.isMobileOpen ? 'opacity-100' : 'opacity-0'} w-full lg:absolute lg:left-7 transition-all duration-300 delay-75 ease-out whitespace-nowrap`}>
                                                {item.name}
                                            </p>
                                        </Link>
                                    </div>
            ...
            <div className='sticky -bottom-4 -mb-5 lg:mb-0 lg:bottom-0 py-5 z-10 bg-[#111] w-full'>
                <button className={`flex items-center gap-2.5 transition-all text-white bg-white/10 
                    ...
                    duration-300 ${sidebar.isOpen || dropdown.isMobileOpen ? 'delay-300' : 'delay-0'}`} onClick={e => e.stopPropagation()}
                    id="specialButton"
                    ...
                    <p style={{transition: 'all 0.4s ease-out', transitionDelay: `${sidebar.isOpen || dropdown.isMobileOpen ? 1 * 50 + 200 : 0}ms`}}
                        className={`${sidebar.isOpen || dropdown.isMobileOpen ? 'opacity-100' : 'opacity-0'}  lg:left-7 transition-all duration-300 delay-75 ease-out whitespace-nowrap`}>
